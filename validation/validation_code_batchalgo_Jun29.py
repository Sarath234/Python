...
#        fw.write('%s\n' %vm01)
        for j in range(0,argmt[i]+1): # +1 modified
            if i==0:
...
#        os.remove(fil_nam_freeVMs)
        ...
        fw=open(fil_nam_freeVMs,'r')
        ips_in_file=fw.read().split('\n')
        fw.close()
#        print ips_in_file[0]
        ssh=paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(ips_in_file[0])
        ssh.exec_command('mpiexec -f /export/user/validation_files/'+fil_nam_freeVMs+' -n'+' '+str(argmt[i]+1)+' python mulpar_new.py /export/user/validation_files/'+s[i]' > /export/user/validation/'+s[i]+'_delay.txt')
...
#            print s,'\n',dt,'\n',tim
            ...
            while no_of_freeVMs<len(s)*2: # *2 modified value
...
#            print 'C matrix= \n', C
            ...
            for l in range(0,C.shape[0]):
                ...
                y=y+argmt[l]+1 # +1 modified value
#            print 'minv= ',minv
#            print 'y', y
#            print 'argmnt', argmt
            freeVMs=free_vms_update()
...
#                    os.remove(fil_nam_freeVMs)
                ...
                for i in range(0,len(argmt)):
                    if argmnt[i]==1:
                        ...
                        min_[i]=C[i,armnt[i]-2]-C[i,argmt[i]-1]
